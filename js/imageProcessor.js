...
window.imageProcessor = {
   ...
   */
  applyChromaKey(ctx, width, height, chromaHex, tolerance, softEdge) {
    const imageData = ctx.getImageData(0, 0, width, height);
    const data = imageData.data;
    const chroma = this.hexToRgb(chromaHex);
   ...
   */
  extractBoundingBoxes(imageData, noiseThreshold = 16) {
    ...
    const queue = new Int32Array(width * height * 2); // Preallocated queue coordinates [x, y...]
    let head = 0;
    let tail = 0;
   ...
   */
  groupBoxesIntoStates(boxes, states) {
